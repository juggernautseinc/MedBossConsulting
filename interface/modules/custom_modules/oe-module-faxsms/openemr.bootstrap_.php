...
function oe_module_faxsms_sms_render_javascript_telehealth_load(Event $event) {
    ?>
    function sendTeleSMS(phone) {
    let btnClose = <?php echo xlj("Cancel"); ?>;
    let title = <?php echo xlj("Send SMS Message"); ?>;
    let url = top.webroot_url + '/interface/modules/custom_modules/oe-module-faxsms/contact_tele.php?isSMS=1&recipient=' + encodeURIComponent(phone);
    dlgopen(url, '', 'modal-md', 600, '', title, {
    buttons: [{text: btnClose, close: true, style: 'secondary'}]
    });
    }
...
$eventDispatcher->addListener(SendSmsEvent::JAVASCRIPT_SMS_TELEHEALTH, 'oe_module_faxsms_sms_render_javascript_telehealth_load');
...
// function
if ($oesms) {
    $smsSendDispatcher->dispatch(SendSmsEvent::JAVASCRIPT_SMS_TELEHEALTH, new GenericEvent());
}
?>
