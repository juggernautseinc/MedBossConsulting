    ...
    <div class="form-group">
            ...
            if ($GLOBALS['portal_onsite_two_enable'] && $GLOBALS['portal_onsite_two_address']) {
                ...
                if ($portalStatus['allow_patient_portal'] == 'YES') {
                    ...
                    <button type='button' class='btn btn-secondary btn-sm' onclick='assignPatientDocuments(<?php echo js_url($pid) ?>)'><?php echo xlt('Assign Documents') ?></button>
                    <?php
                } else {
            ...
            if (!($portalUserSetting)) { // Show that the patient has not authorized portal access ?>
                ...
                <?php $oefax = !empty($GLOBALS['oefax_enable']) ? $GLOBALS['oefax_enable'] : 0;
            }
                if ($oefax) {
