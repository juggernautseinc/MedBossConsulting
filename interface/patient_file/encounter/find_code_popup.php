    ...
    <div class="container-fluid">
        ...
        <?php } ?>
            ...
            <div clas="row align-items-start">
                <?php if ($codetype == 'ICD10' && !empty($pid)) { ?>
                    <div class="col">
                        ...
                        $list_previous = sqlStatement($previous_cpt, [$pid, 'ICD10']);
                        while ($row = sqlFetchArray($list_previous)) {
                            $codeText = explode("\r", $row['code_text']);
                            echo '<a href="" onclick="return selcode(' . attr_js('ICD10') . ', ' . attr_js($row['code']) . ', ' . attr_js(' ') . ', ' . attr_js(trim($codeText[0])) . ')">' . text($row['code']) . ' ' . text($row['code_text']) . '</a><br>';
                        }
                        ?>
                    </div>
                <?php } elseif($codetype == 'CPT4' && !empty($pid)) { ?>
                    <div class="col">
                        <h4>Previous CPT Codes</h4>
                        <?php
                        //get previous diagnosis
                        $previous_cpt = "select distinct code, code_text from billing where pid = ? and code_type = ? ORDER BY id ASC";
                        $list_previous = sqlStatement($previous_cpt, [$pid, 'CPT4']);
                        while ($row = sqlFetchArray($list_previous)) {
                            $codeText = explode("\r", $row['code_text']);
                            echo '<a href="" onclick="return selcode(' . attr_js('CPT4') . ', ' . attr_js($row['code']) . ', ' . attr_js(' ') . ', ' . attr_js(trim($codeText[0])) . ')">' . text($row['code']) . ' ' . text($row['code_text']) . '</a><br>';
                        }
                        ?>
                    </div>
